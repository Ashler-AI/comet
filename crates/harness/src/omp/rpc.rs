//! Minimal JSON-RPC 2.0 client over the ACP agent's stdio (newline-delimited
...
/// A non-response line from the ACP agent, in stdout order.
#[derive(Debug)]
pub(crate) enum Incoming {
    ...
    /// stdout EOF: the ACP agent exited. All pending requests fail.
    Eof,
}
    ...
    pub async fn request(&self, method: &str, params: Value) -> Result<Value, HarnessError> {
        ...
        match rx.await {
            ...
            Err(_) => Err(HarnessError::Protocol(format!(
                "{method}: ACP agent exited before responding"
            ))),
        }
    }
...
async fn write_loop(mut stdin: ChildStdin, mut rx: mpsc::UnboundedReceiver<String>) {
    while let Some(line) = rx.recv().await {
        ...
        if let Err(e) = write.await {
            tracing::debug!(target: "comet_harness::omp", "stdin write failed (tolerated): {e}");
            return;
        }
    }
}
...
async fn read_loop(stdout: ChildStdout, pending: Pending, tx: mpsc::Sender<Incoming>) {
    ...
    while let Ok(Some(line)) = lines.next_line().await {
        ...
        let Ok(msg) = serde_json::from_str::<Value>(line) else {
            tracing::debug!(target: "comet_harness::omp", "non-JSON stdout line (skipped)");
            continue;
        };
