...
impl EngineRpc {
    ...
    async fn admit_scaffold_handoff(
        ...
        let requested_command_id = crate::new_id();
        let mut admitted = false;
        let queued = self
            .doc_host
            .queue_prepared_command(
                &chat_id,
            ...
            .await;
        if admitted && let Some(outcome) = outcome.as_mut() { outcome.armed = false; }
        let command_id = queued.map(|entry| entry.id).map_err(|error| {
            let admission = if admitted { "initial command admitted; metadata update failed" }
                else { "initial command not admitted" };
            RpcError::Failed(format!("{error}; sandbox {sandbox_id}; session {chat_id}; {admission}"))
        })?;
