...
/// Coarse cell loaders use 15Hz while keeping the same wall-clock phase and
/// period. Text dissolves and smooth arc spinners use the full 30Hz clock.
...
/// Renew only while painting an active cosmetic animation. Hidden or removed
/// views stop renewing, so the clock parks after the last lease expires.
        ...
        cx.spawn(async move |cx| {
            ...
            loop {
                ...
                let parked = cx.update(|cx| {
                    ...
                    clock
                        .leases
                        .retain(|_, lease| !reduced && lease.until > now);
