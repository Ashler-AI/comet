...
| `png/crew-icon-macos-1024.png` | Raster of the squircle tile. Feeds the `.icns` in `scripts/package-macos.sh`; committed rather than rendered at package time so a release never depends on an SVG rasterizer being present on the runner. |
...
`rsvg-convert`/`inkscape` on hand, stock macOS will do it:
